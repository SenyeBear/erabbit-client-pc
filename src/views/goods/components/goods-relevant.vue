...
export default {
  ...
  setup (props) {
    // 准备轮播图需要的sliders数据 格式[[4个],[4],[4],[4]]
    const sliders = ref([])
    findRelevantGoods({ id: props.goodsId }).then(data => {
      // console.log(data.result) [{},{},{},...,{}] 16条
      // 改造数据 分页
      // 每页几个
      const pageSize = 4
      // 总共几页
      const totalPage = Math.ceil(data.result.length / pageSize)
      // 遍历每一页
      for (let i = 0; i < totalPage; i++) { // i代表当前第几页 0-4 4-8...
